...
Int ScreenBWFilter::set(FilterModes mode)
{
	...
	{	//rendering a quad with redirected rendering surface tinted by pixel shader
		...
		{	//turning effect off
			...
			{	m_curFadeValue = 0.0f; hr = W3DShaderManager::LoadAndCreateD3DShader("shaders\\monochrome.pso", Declaration, 0, false, (void**)&m_dwBWPixelShader);
		...
		hr=DX8Wrapper::_Get_D3D_Device8()->SetPixelShader((DWORD)m_dwBWPixelShader);
...
{
	if (m_dwBWPixelShader) { m_dwBWPixelShader->Release(); m_dwBWPixelShader = nullptr; }
...
Int TerrainShaderPixelShader::init( void )
{
	...
	if (terrainShader2Stage.init() && (res=W3DShaderManager::getChipset()) >= DC_GENERIC_PIXEL_SHADER_1_1)
	{
		if (res >= DC_GENERIC_PIXEL_SHADER_1_1)
		{
			//this shader needs some assets that need to be loaded
			//shader decleration
			// shader declaration - Ronin @build 29/10/2025 DX9: Use D3DVERTEXELEMENT9 instead of D3DVSD_ * macros
				D3DVERTEXELEMENT9 Declaration[] = {
					{0, 0,  D3DDECLTYPE_FLOAT3,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITION, 0},
					{0, 12, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR,    0},
					{0, 16, D3DDECLTYPE_FLOAT2,   D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_TEXCOORD, 0},
