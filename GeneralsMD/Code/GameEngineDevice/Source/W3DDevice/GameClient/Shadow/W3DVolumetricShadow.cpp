...
void W3DVolumetricShadowManager::renderShadows( Bool forceStencilFill )
{
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
		...
		// Ronin @debug 15/12/2025: Capture state BEFORE shadow rendering
/*#ifdef _DEBUG
		DWORD preFVF = 0;
		IDirect3DVertexDeclaration9* preDecl = nullptr;
		IDirect3DVertexBuffer9* preVB = nullptr;
		UINT preOffset = 0, preStride = 0;
		m_pDev->GetFVF(&preFVF);
		m_pDev->GetVertexDeclaration(&preDecl);
		m_pDev->GetStreamSource(0, &preVB, &preOffset, &preStride);
		WWDEBUG_SAY(("🌑 SHADOW renderShadows START: FVF=0x%08X, Decl=%p, VB=%p, Stride=%u",
			preFVF, preDecl, preVB, preStride));
		if (preDecl) preDecl->Release();
		if (preVB) preVB->Release();
#endif*/
...
//		m_pDev->SetRenderState(D3DRS_ZBIAS,1);	///@todo: See if this helps or makes things worse.
		...
		// Ronin @debug 15/12/2025: Check final FVF state before cleanup
		/*
#ifdef _DEBUG
		DWORD finalCheckFVF = 0;
		m_pDev->GetFVF(&finalCheckFVF);
		WWDEBUG_SAY(("🌑 SHADOW: After all rendering, final FVF=0x%08X (expected 0x%02X)",
			finalCheckFVF, SHADOW_DYNAMIC_VOLUME_FVF));
#endif*/
...
//		m_pDev->SetRenderState(D3DRS_ZBIAS,0);	///@todo: See if this helps or makes things worse.
		...
		// Ronin @bugfix 16/12/2025: CRITICAL SHADOW ATLAS FIX
...
//		if (numRenderedShadows)
		...
		// Ronin @bugfix 15/12/2025: Reset world transform via wrapper before invalidation
		Matrix3D identity(true);
		DX8Wrapper::Set_Transform(D3DTS_WORLD, identity);

		DX8Wrapper::Invalidate_Cached_Render_States();

		// Ronin @debug 15/12/2025: Capture state AFTER Invalidate_Cached_Render_States
		/*
#ifdef _DEBUG
		DWORD finalFVF = 0;
		IDirect3DVertexDeclaration9* finalDecl = nullptr;
		m_pDev->GetFVF(&finalFVF);
		m_pDev->GetVertexDeclaration(&finalDecl);
		WWDEBUG_SAY(("🌑 SHADOW renderShadows END (post-Invalidate): FVF=0x%08X, Decl=%p",
			finalFVF, finalDecl));
		if (finalDecl) finalDecl->Release();
#endif*/
	}
	else
	if (forceStencilFill)
