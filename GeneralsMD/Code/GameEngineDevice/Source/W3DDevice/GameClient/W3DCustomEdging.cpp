...
void W3DCustomEdging::loadEdgingsInVertexAndIndexBuffers(WorldHeightMap* pMap, Int minX, Int maxX, Int minY, Int maxY,
	Int terrainTexturePage, Int edgeTexturePage)
{
	if (!m_indexEdging || !m_vertexEdging || !m_initialized) {
		return;
	}
	if (!m_anythingChanged &&
		m_loadedTerrainTexturePage == terrainTexturePage &&
		m_loadedEdgeTexturePage == edgeTexturePage) {
		return;
	}
	...
	for (row = minY; row < maxY - 1; row++) {
		for (column = minX; column < maxX - 1; column++) {
			...
			const Int blendTerrainTexturePage = pMap->getTerrainTexturePageForTileNdx(pMap->m_blendedTiles[blend].blendNdx);
			const Int customEdgeTexturePage = pMap->getEdgeTexturePageForBlendClass(pMap->m_blendedTiles[blend].customBlendEdgeClass);
			if (terrainTexturePage >= 0 && blendTerrainTexturePage != terrainTexturePage) continue;
			if (edgeTexturePage >= 0 && customEdgeTexturePage != edgeTexturePage) continue;
...
W3DCustomEdging::W3DCustomEdging()
{
	...
	m_loadedTerrainTexturePage = -1;
	m_loadedEdgeTexturePage = -1;
	clearAllEdging();
	...
	TextureClass* terrainTexture, TextureClass* cloudTexture, TextureClass* noiseTexture)
{
	...
	const Bool usePagePairBatching = pMap->getTerrainTexturePageCount() > 1 || pMap->getEdgeTexturePageCount() > 1;

	// @refactor Ronin 09/04/2026 Consolidate repeated custom edging render-state setup without changing batching behavior.
	const auto drawCurrentBatch =
		[&](TextureClass* pTerrainTexture, TextureClass* pEdgeTexture)
		{
			if (pTerrainTexture == nullptr || pEdgeTexture == nullptr) {
				return;
			}

			const auto drawBatchTriangles =
				[&]()
				{
					DX8Wrapper::Draw_Triangles(m_curEdgingIndexOffset, m_curNumEdgingIndices / 3, 0, m_curNumEdgingVertices);
				};

			const auto applyAlphaTestState =
				[&](UnsignedInt alphaRef, D3DCMPFUNC alphaFunc)
				{
					DX8Wrapper::Set_DX8_Render_State(D3DRS_ALPHAREF, alphaRef);
					DX8Wrapper::Set_DX8_Render_State(D3DRS_ALPHAFUNC, alphaFunc);
					DX8Wrapper::Set_DX8_Render_State(D3DRS_ALPHATESTENABLE, true);
				};

			const auto applyOverlayTextureStages =
				[&]()
				{
			...
			const auto applyOverlayBlendState =
				[&]()
				{
			...
			const auto drawOverlayPass =
				[&](TextureClass* pOverlayTexture)
				{
					...
					DX8Wrapper::Set_Texture(0, pOverlayTexture);
					DX8Wrapper::Set_Texture(1, pEdgeTexture);
					DX8Wrapper::Apply_Render_State_Changes();

					applyOverlayTextureStages();
					applyAlphaTestState(0x80, D3DCMP_NOTEQUAL);
			...
			DX8Wrapper::Set_Texture(0, pTerrainTexture);
			DX8Wrapper::Set_Texture(1, pEdgeTexture);
			DX8Wrapper::Apply_Render_State_Changes();

			applyAlphaTestState(0x7B, D3DCMP_LESSEQUAL);
			drawBatchTriangles();

			DX8Wrapper::Set_Texture(0, pEdgeTexture);
			DX8Wrapper::Set_Texture(1, nullptr);
			DX8Wrapper::Apply_Render_State_Changes();

			applyAlphaTestState(0x84, D3DCMP_GREATEREQUAL);
			drawBatchTriangles();
			...
			drawOverlayPass(cloudTexture);
			drawOverlayPass(noiseTexture);
		};

	if (!usePagePairBatching) {
		...
		drawCurrentBatch(terrainTexture, pMap->getEdgeTerrainTexture());
		return;
	}
	...
	std::vector<EdgingTexturePagePair> pagePairs;
	pagePairs.reserve(8);

	for (Int row = minY; row < maxY - 1; ++row) {
		for (Int column = minX; column < maxX - 1; ++column) {
			const Int cellNdx = column + row * pMap->getXExtent();
			const Int blend = pMap->m_blendTileNdxes[cellNdx];
			if (blend == 0) continue;

			const Int edgeClass = pMap->m_blendedTiles[blend].customBlendEdgeClass;
			if (edgeClass < 0) continue;

			const Int terrainPage = pMap->getTerrainTexturePageForTileNdx(pMap->m_blendedTiles[blend].blendNdx);
			const Int edgePage = pMap->getEdgeTexturePageForBlendClass(edgeClass);

			Bool exists = false;
			for (Int pairIndex = 0; pairIndex < (Int)pagePairs.size(); ++pairIndex) {
				if (pagePairs[pairIndex].terrainPage == terrainPage &&
					pagePairs[pairIndex].edgePage == edgePage) {
					exists = true;
					break;
				}
			}
			...
			if (!exists) {
				EdgingTexturePagePair pair;
				pair.terrainPage = terrainPage;
				pair.edgePage = edgePage;
				pagePairs.push_back(pair);
			}
		}
	}

	for (Int pairIndex = 0; pairIndex < (Int)pagePairs.size(); ++pairIndex) {
		const EdgingTexturePagePair& pair = pagePairs[pairIndex];
		loadEdgingsInVertexAndIndexBuffers(pMap, minX, maxX, minY, maxY, pair.terrainPage, pair.edgePage);
		if (m_curNumEdgingIndices == 0) {
			continue;
		}

		TextureClass* pagedTerrainTexture = pMap->getTerrainTexture(pair.terrainPage);
		if (pair.terrainPage == 0 && terrainTexture != nullptr) {
			pagedTerrainTexture = terrainTexture;
		}

		TextureClass* pagedEdgeTexture = pMap->getEdgeTerrainTexture(pair.edgePage);
		drawCurrentBatch(pagedTerrainTexture, pagedEdgeTexture);
	}
}
